...
int ddos_detector(void *ctx) {
    ...
    __u64 present = bpf_ktime_get_ns();
    
    if((present - *ddos_detector_timer) > 1e10){
        detector_timer_map.update(&ddos_detector_timer_index, &ddos_detector_timer_init_value);
        packets_number_map.update(&receive_packets_index, &receive_packets_init_value);
    }

    __u64 rpn = *receive_packets_number;
    rpn++;

    if(rpn > max_packets_number_per_ten_seconds){
        bpf_trace_printk("Detect DDOS!!! => number of packets in ten seconds : %llu\\n", rpn);
    }
    
    receive_packets_number = &rpn;
